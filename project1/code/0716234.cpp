...
int main(){
    ...
    expression += " and dst host 140.113.0.2"; // add default filtering rule
    ...
    system("ip link set BRGrGWr master br0");
    ...
    for(int i = 1;; ){
        ...
        if(ret == 1) { // if success
			...
			//dump outer udp
			struct udphdr *udp = (struct udphdr *)(content + ETHER_HDR_LEN + ip_header_len);
			...
			printf("Outer Source port: %u\n", source_port);
            printf("Outer Destination port: %u\n", destination_port);
            printf("Next Layer Protocol: GRE\n");
			
            //dump gre
            struct gre_base_hdr *gre = (struct gre_base_hdr *)(content + ETHER_HDR_LEN + ip_header_len + 8);
            u_int16_t protocol = ntohs(gre->protocol);
			u_int32_t key = ntohl(gre->key);
            printf("Protocol: %x\n", protocol);
			printf("Key: %u\n", key);

            //dump inner ethernet
            struct ether_header *ethernet_i = (struct ether_header *)(content + ETHER_HDR_LEN + ip_header_len + 8 + 8);
            ...
            if(blacklist.find(std::to_string(source_port)) == blacklist.end()){
                blacklist[std::to_string(source_port)] = true;
                struct bpf_program command;
                for(auto &it : blacklist){
                    expression += (" and not src port " + it.first);
                }
                ...
                string c1 = "ip link add " + gretap  + " type gretap remote " + src_ip + " local 140.113.0.2 key " + std::to_string(key)
					+ " encap fou encap-sport " + std::to_string(destination_port) + " encap-dport " + std::to_string(source_port);
                string c2 = "ip link set " + gretap + " up";
                string c3 = "ip link set " + gretap +  " master br0";
				string c4 = "ip fou add port " + std::to_string(destination_port) + " ipproto 47";
